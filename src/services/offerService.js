...
class OfferService {
    ...
    // Opret et nyt tilbud baseret på en CleaningPlan

    async createOffer(planId, { discountPercent = 0, environmentalFeePercent = 1 }) {
        ...
        // 7) Opret signature token til kunden
     ...
     // Send tilbud til kunden (status → sent)
    ...
    async acceptOffer(offerId, { name, email }) {
        const offer = await offerRepo.findById(offerId);
        if (!offer) throw new Error("Tilbud findes ikke");
     ...
     // Hent alle tilbud for en kunde

    async listOffersForCustomer(customerId) {
        return await offerRepo.findByCustomerId(customerId);
    }


    // Hent alle tilbud for en plan

    async listOffersForPlan(planId) {
